...
client_name = my_names.get_full_name()
...
try:
    ...
    while True:
        with open("log.ndjson", 'a') as logfile:
            print("-------------------------Approx. hour of day: "+str(round(time_scaling.get_scaled_time()))+" (24h)-------------------------")
            recv_time = time.time()
            ...
            time.sleep(recv_time+5 - time.time()) # In questo modo si (prova a) mantenere
